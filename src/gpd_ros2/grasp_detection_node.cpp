#include <gpd_ros2/grasp_detection_node.h>
...
void GraspDetectionNode::cloud_indexed_callback(const gpd_ros2::CloudIndexed& msg)
{
  if (!has_cloud_)
  {
...
void GraspDetectionNode::cloud_samples_callback(const gpd_ros2::CloudSamples& msg)
{
  if (!has_cloud_)
  {
    initCloudCamera(msg.cloud_sources);
...
void GraspDetectionNode::samples_callback(const gpd_ros2::SamplesMsg& msg)
{
  if (!has_samples_)
  {
    Eigen::Matrix3Xd samples(3, msg.samples.size());
...
void GraspDetectionNode::initCloudCamera(const gpd_ros2::CloudSources& msg)
{
  // clean up
  delete cloud_camera_;
  cloud_camera_ = NULL;
